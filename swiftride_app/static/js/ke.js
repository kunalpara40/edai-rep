...
// REMOVED: Duplicate loadRideStatus function to avoid conflicts
// The enhanced version later in the file will be used instead
...
// Enhanced loadRideStatus function with better error handling and session management
async function loadRideStatus() {
  if (!currentRideId) return;

  try {
    console.log(`🔄 Loading ride status for ride ID: ${currentRideId}`);
    ...
    if (!response.ok) {
      if (response.status === 401) {
        ...
        console.error('❌ Failed to load ride status:', response.status);
        return;
      }
    }

    const data = await response.json();
    console.log('📊 Ride status loaded:', data);
    updateRideStatusUI(data);
    ...
    if (error instanceof TypeError && error.message === 'Failed to fetch') {
      ...
      alert('Error loading ride status. Please try again.');
    }
  }
}

// Enhanced updateRideStatus function with better error handling
async function updateRideStatus(rideId) {
  try {
    console.log(`🔄 Updating ride status for ride ID: ${rideId}`);
    ...
    if (!response.ok) {
      if (response.status === 401) {
        ...
        console.error('❌ Ride not found');
        // Stop polling for non-existent rides
        if (statusUpdateInterval) {
          clearInterval(statusUpdateInterval);
          statusUpdateInterval = null;
        }
    ...
    const data = await response.json();
    console.log('📊 Ride updates received:', data);
    handleRideStatusUpdate(data.ride);
...
// Enhanced loadUserWalletBalance function with better error handling
async function loadUserWalletBalance(fareAmount) {
  try {
    console.log('💰 Loading user wallet balance');
...
// Enhanced updateNavbarWalletBalance function with better error handling
async function updateNavbarWalletBalance() {
  try {
    console.log('💳 Updating navbar wallet balance');
    ...
    console.log(`💳 Navbar wallet balance updated: ₹${walletBalance.toFixed(2)}`);
